...
**Next task:** 1.6 masters: products, UOM and conversions, product categories, vendors, MPPs and external codes (SAP). PRs #19 (mobile sign-in) and #20 (administration) are open; merge in order. DEV runs the latest `dev` build but has no organisation yet: bootstrap it with `org:bootstrap`, `user:create` and `user:grant` (docs/runbooks/auth.md) when you want to sign in there.
...
- [x] 1.3 Web sign-in (PR #18): sign-in by e-mail or employee code with clear refusals (wrong password, locked with minutes, disabled, too many attempts, offline), forgot / reset (token read from the URL fragment and removed) / change password screens with the server's password rules shown on the field, forced password change after a temporary one, route guards with return to the requested page (same-origin paths only), account menu with name, change password and sign-out. Access token in memory only; silent refresh a minute before expiry, serialised across tabs with the Web Locks API (so tabs never replay a rotated refresh cookie); a refused token is refreshed once and the request repeated; sign-out in one tab signs out all tabs (BroadcastChannel) and clears cached server data. First-load JS 159.0 KB gzip.
- [x] 1.4 Mobile sign-in (PR #19): sign-in by e-mail or employee code, forgot password (link opens the web reset page), change password (forced after a temporary one), account section with sign-out on More. Refresh token in SecureStore (Keystore/Keychain, this device only), access token in memory, session restored on launch (offline launches stay signed in), refused tokens refreshed once and retried, cached data cleared on sign-out. App lock after 5 minutes in the background with the phone's fingerprint/face/PIN (skipped on phones with no screen lock). Routes guarded with Expo Router `Stack.Protected`. All in Expo Go (expo-secure-store, expo-local-authentication, expo-device).
- [x] 1.5 Administration (PR #20). API: users list (search, status, role, location and department filters, cursor pages), create (invited by e-mail, 72 h link), edit with optimistic locking, roles with scopes (store users without a location scope get their own locations), deactivate (signs out at once), unlock, forced password reset (24 h link, password becomes temporary); roles from the permission catalogue; save location, department, designation (the last two added to the contract). Rules: nobody grants a role or builds one beyond their own permissions, the organisation keeps an active super administrator, admins cannot disable themselves. Every change is written to the audit log in the same transaction with a per-organisation, per-day SHA-256 chain; role changes emit `UserRoleChanged`. `pnpm org:bootstrap` sets up a fresh environment. Web: Administration section (users, user detail with roles editor and confirmed actions, roles with grouped permissions, locations, departments and designations), EN/HI. Not yet: "reports to" picker and product-category scopes (need the catalogue, 1.6).
...
### 2026-09-26: web sign-in
- Found a build-cache bug: Turbo cached the web app's GraphQL codegen with inputs that didn't include the web sources, so a new query could be restored as stale generated code. `@ie/web#codegen` now lists its own inputs.
- End-to-end through the web proxy with a cookie jar: sign-in, `me` (114 permissions for the local admin), CSRF-protected refresh, sign-out, refused refresh afterwards.

### 2026-09-26: mobile sign-in
- Shared validation schemas (`@ie/validation`) now run on the phone too.
- The laptop's Wi-Fi address changes between sessions (was 10.235.234.42, now 192.168.71.160); the app follows the bundle host automatically.

### 2026-09-26: DEV re-install, administration
- DEV: re-ran `install.sh DEV`; it replaced the placeholder signing key and the agent deployed the latest `dev` images from GHCR (it had last deployed from the in-VM test registry). Healthy. Its database has no organisation, roles or users yet: bootstrap with `org:bootstrap`, `user:create`, `user:grant`. Booting it needed Docker Desktop and the dev servers stopped (1.7 GB free); DEV was powered off afterwards.
- The earlier "database not ready" readiness blip on the laptop recovered by itself; it coincided with memory under 1 GB.
- Web tests type character by character and hit the 5 s default under full parallel runs on this laptop; the web test timeout is now 15 s.
