...
**Next task:** 1.4 mobile sign-in: refresh token in SecureStore, sign-in screen, session restore on launch, sign-out, app lock after time in the background. PRs #14–#18 are stacked (ui → mobile → auth → authz → web sign-in); merge in order. Before #16 reaches DEV, re-run `infrastructure/deploy/install.sh DEV` (see docs/runbooks/auth.md).
