...
**Next task:** finish Phase 0. First 0.10 `packages/ui` (Radix primitives replacing `apps/web/src/components/ui.tsx`). Then 0.11 the Expo mobile skeleton, which needs a decision on Android tooling: this laptop cannot run an emulator alongside everything else, so use a physical phone with a development build. Then Phase 1 (identity & admin). PRs #10–#13 are open and stacked (web → ship web → worker → lint); merge in order. After #11 merges, add "Web image build and scan" to the required checks.
