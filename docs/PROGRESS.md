...
**Next task:** 1.5 admin console (web): users (create/invite, edit, roles with scopes defaulting store users to their locations, deactivate, unlock, reset password), roles (view, custom roles from the permission catalogue), locations, departments, designations. PRs #14–#19 are stacked (ui → mobile → auth → authz → web sign-in → mobile sign-in); merge in order. Before #16 reaches DEV, re-run `infrastructure/deploy/install.sh DEV` (see docs/runbooks/auth.md).
